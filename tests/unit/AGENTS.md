...
| `windowBounds.test.ts`           | Tests `@/process/utils/windowBounds` `resolveInitialBounds`/`loadSavedWindowBounds`/`attachWindowBoundsPersistence` decision tree (saved vs proportional defaults, MIN*WINDOW*\* clamps) with stubbed `electron.screen` displays.                         |
