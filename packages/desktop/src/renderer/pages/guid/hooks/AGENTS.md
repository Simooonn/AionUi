...
- Agent identity flows through `getAgentKey` (`agentSelectionUtils.ts`); reuse it rather than re-deriving keys. Note preset *assistants* use a separate `custom:<assistantId>` form produced by `AssistantSelectionArea`, not `AgentRegistry`.
