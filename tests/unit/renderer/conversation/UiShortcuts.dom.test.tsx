...
/**
 * Stand-in for ChatLayout's ownership of the right-panel collapse state:
 * useWorkspaceCollapse no longer self-manages state or listens for
 * WORKSPACE_TOGGLE_EVENT (that moved to ChatLayout), so tests exercising the
 * Cmd/Ctrl+L integration need a minimal owner that reproduces the same
 * "handled" preventDefault contract.
 */
const useWorkspaceCollapseOwner = (workspaceEnabled: boolean) => {
  const [rightSiderCollapsed, setRightSiderCollapsed] = React.useState(true);
  React.useEffect(() => {
  ...
  const applyRightCollapsed = React.useCallback((collapsed: boolean) => setRightSiderCollapsed(collapsed), []);
  return { rightSiderCollapsed, applyRightCollapsed };
};
...
describe('common desktop UI shortcuts', () => {
  ...
  it('uses the enabled workspace consumer and preserves its collapse state owner', () => {
    const { result } = renderHook(() => {
      const owner = useWorkspaceCollapseOwner(true);
      const workspace = useWorkspaceCollapse({
        workspaceEnabled: true,
        isMobile: false,
        conversation_id: 'conversation-1',
        rightSiderCollapsed: owner.rightSiderCollapsed,
        applyRightCollapsed: owner.applyRightCollapsed,
      });
      useConversationShortcuts({
  ...
  it('leaves Cmd/Ctrl+L untouched when the conversation has no enabled workspace', () => {
    const { result } = renderHook(() => {
      const owner = useWorkspaceCollapseOwner(false);
      const workspace = useWorkspaceCollapse({
